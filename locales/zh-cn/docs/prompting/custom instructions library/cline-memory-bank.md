...
-   **这套指令旨在实现什么目标？**
    ...
    -   这套指令将 Cline 转变为一个自我记录的开发系统，通过结构化的“记忆库”在不同会话间保持上下文。它确保一致的文档记录、对变更的仔细验证以及与用户的清晰沟通。

-   **这最适合哪些类型的项目或任务？**
    -   需要广泛上下文跟踪的项目。
    -   任何项目，不论技术栈（技术栈详情存储在 `techContext.md` 中）。
    -   正在进行和新项目。
...
-   **最佳实践**
    -   在操作过程中监控 `[记忆库：激活]` 标志。
    -   对关键操作进行信心检查。
    -   开始新项目时，为 Cline 创建项目简介（粘贴到聊天中或包含在 `cline_docs` 中的 `projectBrief.md` 中），用于创建初始上下文文件。
...
重要：如果 `cline_docs/` 或这些文件不存在，请立即创建它们，通过：
