...
将 MCP 服务器视为特殊的助手，它们赋予 Cline 额外的能力！它们让 Cline 能够执行酷炫的操作，比如获取网页或处理你的文件。
...
现在有一些好消息。虽然不完美，但您可以让 Cline 相当可靠地为后续的服务器安装执行此操作。在 Cline 设置中（右上角工具栏按钮）的“自定义指令”中添加以下内容：
