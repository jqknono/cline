...
MCP服务器作为大型语言模型（LLM，如Claude）与外部工具或数据源之间的中介。它们是暴露功能给LLM的小程序，使它们可以通过MCP与外部世界进行交互。MCP服务器本质上就像LLM可以使用的API。
...
-   **自然语言理解：**用自然语言向Cline指示构建MCP服务器，描述其功能。Cline将解释您的指令并生成必要的代码。
...
- **动态集成：** 结合多个 MCP 服务器功能以执行复杂任务。例如，Cline 可以使用 GitHub 服务器获取数据，并使用 Notion 服务器创建格式化报告。
