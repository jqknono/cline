...
-   **开发工具：** 编程必备软件（Homebrew、Node.js、Git 等）。
    -   按照我们的 [安装必备开发工具](installing-dev-essentials.md) 指南在 Cline 的帮助下进行设置（在完成此处的设置后）
    -   Cline 将指导您安装所需的一切
-   **Cline 项目文件夹：** 专用于所有 Cline 项目的文件夹。
    ...
    -   在这个 Cline 文件夹内，为每个项目创建单独的文件夹
        ...
        -   示例：`Documents/Cline/portfolio-website` 用于您的投资组合网站
-   **VS Code 中的 Cline 扩展：** 在 VS Code 中安装的 Cline 扩展。

-   这里有一个 [教程](https://www.youtube.com/watch?v=N4td-fKhsOQ) 关于您开始所需的一切。
...
6. **打开 Cline：** 安装后，您可以通过几种方式打开 Cline：
    - 点击活动栏中的 Cline 图标。
    - 使用命令面板（`CMD/CTRL + Shift + P`）并输入 "Cline: 在新标签页中打开"，以在编辑器中以标签页形式打开 Cline。这是为了获得更好的视图而推荐的。
    - **故障排除：** 如果您没有看到 Cline 图标，请尝试重新启动 VS Code。
    - **您将看到什么：** 您应该会在您的 VS Code 编辑器中看到 Cline 聊天窗口出现。
...
2.  **输入您的 OpenRouter API 密钥：**
    ...
    -   选择您偏好的 API 模型。
        -   **推荐用于编程的模型：**
            ...
            -   `google/gemini-2.0-flash-exp:free`：编程的免费选项。
...
```
Hey Cline! 你能帮我在我的 Cline 目录中创建一个名为 "hello-world" 的新项目文件夹，并制作一个显示 "Hello World" 的大蓝色文本的简单网页吗？
```
...
-   **使用屏幕截图：** Cline 可以理解图像，所以请随意使用屏幕截图来展示您正在处理的内容。
